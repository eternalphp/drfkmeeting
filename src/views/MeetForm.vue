...
<van-overlay :show="showOverlay"  @click="showOverlay = false">
    <van-button class="reg-btn" type="primary" :url="regUrl" block v-trigger>报名参会授权</van-button>
</van-overlay>
...
<van-popup v-model="showJobTitle" position="bottom">
  <van-picker
    show-toolbar
    :columns="jobtitleList"
    @confirm="jobtitleConfirm"
    @cancel="showJobTitle = false"
  />
</van-popup>
...
<script>
import Vue from 'vue';
import {GetUserInfo, AddOrUpdate} from '@/api/user'
import area from '@/utils/area';
import { Form,Field,Button,RadioGroup, Radio,Checkbox, CheckboxGroup,Overlay,Notify,Area,Popup, Picker,Dialog,Empty ,Toast } from 'vant';
        ...
        if(code){
          GetUserInfo(code).then((resp)=>{
            ...
            if(resp.success == false){
              this.errorPage = true;
              Notify({ type: 'danger', message: `错误信息:${resp.msg}` });
            }else{
